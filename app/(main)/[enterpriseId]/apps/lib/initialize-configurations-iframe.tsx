 ...
 * @param packageName - 選択したアプリのパッケージ名
 * @param onSuccess - 初期化成功時のコールバック関数
 * @returns boolean - 初期化成功かどうか
 */
export const initializeConfigurationsIframe = ({
  token,
  containerRef,
  enterpriseId,
  appType,
  ...
  const iframeUrl = `${CONFIGURATIONS_IFRAME_CONFIG.BASE_URL}?token=${token}&locale=${CONFIGURATIONS_IFRAME_CONFIG.LOCALE}&packageName=${packageName}`;
  // console.log("iframeUrl", iframeUrl);
  try {
    const options = {
      url: iframeUrl,
      where: containerRef.current,
      attributes: CONFIGURATIONS_IFRAME_CONFIG.IFRAME_STYLE,
    };
